"""
Fine-tuning the library models 
for language modeling on a text file (GPT, GPT-2, BERT, RoBERTa).
    ...
    for example_index, example in enumerate(examples):
        ...
        if example_index < 5:
            if stage == "train":
                ...
                logger.info("source_ids: %s", " ".join(map(str, source_ids)))
                logger.info("source_mask: %s", " ".join(map(str, source_mask)))

                logger.info(
                    "target_tokens: %s",
                    [x.replace("\u0120", "_") for x in target_tokens],
                )
                logger.info("target_ids: %s", " ".join(map(str, target_ids)))
                logger.info("target_mask: %s", " ".join(map(str, target_mask)))
...
def main():
    ...
    if args.do_train:
        ...
        progress_bar = tqdm(
            range(num_train_optimization_steps), total=num_train_optimization_steps
        )
        ...
        for _ in progress_bar:
            ...
            progress_bar.set_description(f"loss {train_loss}")
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            predictions = []
            with open(
                os.path.join(args.output_dir, f"test_{idx}.output", "w")
            ) as f, open(os.path.join(args.output_dir, f"test_{idx}.gold", "w")) as f1:
                for ref, gold in zip(p, eval_examples):
                    predictions.append(str(gold.idx) + "\t" + ref)
                    f.write(str(gold.idx) + "\t" + ref + "\n")
                    f1.write(str(gold.idx) + "\t" + gold.target + "\n")
            ...
            (goldMap, predictionMap) = bleu.computeMaps(
                predictions, os.path.join(args.output_dir, f"test_{idx}.gold")
            )
            dev_bleu = round(bleu.bleuFromMaps(goldMap, predictionMap)[0], 2)
            logger.info("  %s = %s ", "bleu-4", str(dev_bleu))
            logger.info(separator)
